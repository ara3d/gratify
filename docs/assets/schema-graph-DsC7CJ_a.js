import{p as w,r as p,v as d,c as u,h as M,m as O,u as R,A as T,S as D,L as g,R as P,d as _}from"./runtime-DNEt0yeT.js";import{w as I}from"./middleware-DnFxgLOc.js";import{B as l}from"./widgets-BUxxEPAc.js";import{S as A,t as y,l as q,a as c,b as z,F as L,T as F,c as x,s as W,n as V,d as C}from"./parts-CY1Ouhp6.js";import{a as N}from"./source-panel-CSqvtNlY.js";import"./effects-BhoHrEqA.js";function B(e,t){const n=Math.min(t.w/Math.max(1,e.w),t.h/Math.max(1,e.h));return{scale:n,ox:t.x+(t.w-e.w*n)/2-e.x*n,oy:t.y+(t.h-e.h*n)/2-e.y*n}}const $=(e,t)=>{if(!e)return t;const n=Math.min(e.x,t.x),s=Math.min(e.y,t.y);return p(n,s,Math.max(e.right,t.right)-n,Math.max(e.bottom,t.bottom)-s)},H=w("minimap").props().defaults({width:180,height:120}).size(e=>d(e.width,e.height)).style((e,t)=>({fill:u(e.bg,.8+.15*t.hover),edge:e.mix(e.muted,e.accent,.4*t.hover),node:e.muted,view:u(e.accent,.9),viewFill:u(e.accent,.08)})).render((e,t,n)=>{const s=e.rect,r=e.view;if(t.box(s,8,n.fill,n.edge,1),!r)return;const a=p(-r.pan.x/r.zoom,-r.pan.y/r.zoom,r.w/r.zoom,r.h/r.zoom),S=e.props.items.reduce((o,m)=>$(o,m.rect),a),i=B(S,s.inset(8)),f=o=>d(i.ox+o.x*i.scale,i.oy+o.y*i.scale);for(const o of e.props.items){const m=f(d(o.rect.x,o.rect.y)),E=o.hue===void 0?n.node:M(o.hue,.6,o.selected?.7:.5);t.box(p(m.x,m.y,Math.max(2,o.rect.w*i.scale),Math.max(2,o.rect.h*i.scale)),1,E)}const b=f(d(a.x,a.y));t.box(p(b.x,b.y,a.w*i.scale,a.h*i.scale),2,n.viewFill,n.view,1)}).semantics(e=>({role:"img",label:`minimap of ${e.props.items.length} nodes`})),j=w("dock").props().defaults({corner:"bottom-right",margin:12}).fill().arrange((e,t,n)=>n.map(s=>{const r=e.corner.endsWith("right")?t.right-e.margin-s.size.x:t.x+e.margin,a=e.corner.startsWith("bottom")?t.bottom-e.margin-s.size.y:t.y+e.margin;return p(r,a,s.size.x,s.size.y)})),K=`// ============================================================================
...
`,G=`// The parts of the schema graph: a table node with one port per column, the
...
`,U=`// marquee.ts — rubber-band selection as ONE reusable gesture, zero framework
...
`,Y=`// Pure layered graph layout ("arrange"): assign each node a column by the
...
`,X=`// Minimap — a screen-layer overview of a world-space graph: every node as a
...
`,J=`// A small sample database for the browser examples: nine tables with typed
...
`,h=(e,t)=>({...e,selected:t,selectedEdge:null});function k(e){const t=Object.values(e.tables).map(r=>({id:r.id,w:y(r.columns.length).x,h:y(r.columns.length).y})),n=q(t,e.edges.map(r=>({from:c(r.from),to:c(r.to)})),{gapX:90,gapY:36,origin:d(60,150)}),s=Object.fromEntries(Object.values(e.tables).map(r=>[r.id,{...r,pos:n.get(r.id)??r.pos}]));return{...e,tables:s}}function Q(e,t){switch(t.kind){case"select":{const n=new Set(t.mods.ctrl||t.mods.shift?e.selected:[]);return t.mods.ctrl&&n.has(t.id)?n.delete(t.id):n.add(t.id),h(e,n)}case"marquee":{const n=new Set(t.mods.shift?e.selected:[]);for(const s of Object.values(e.tables))x(s).overlaps(t.rect)&&n.add(s.id);return h(e,n)}case"drag":{const n=e.selected.has(t.id)?e.selected:new Set([t.id]),s={...e.tables};for(const r of n)s[r]={...s[r],pos:d(s[r].pos.x+t.delta.x,s[r].pos.y+t.delta.y)};return{...e,tables:s}}case"connect":{const[n,s]=W(t.a)==="out"?[t.a,t.b]:[t.b,t.a];if(c(n)===c(s))return e;const r=e.edges.filter(a=>a.from!==n);return{...e,edges:[...r,{id:V(),from:n,to:s}]}}case"select-edge":return{...e,selectedEdge:t.id,selected:new Set};case"delete":{if(e.selectedEdge)return{...e,edges:e.edges.filter(r=>r.id!==e.selectedEdge),selectedEdge:null};if(!e.selected.size)return e;const n=Object.fromEntries(Object.entries(e.tables).filter(([r])=>!e.selected.has(r))),s=e.edges.filter(r=>!e.selected.has(c(r.from))&&!e.selected.has(c(r.to)));return{...e,tables:n,edges:s,selected:new Set}}case"clear":return h(e,new Set);case"select-all":return h(e,new Set(Object.keys(e.tables)));case"arrange":return k(e);case"reset":return v}}const v=k({tables:Object.fromEntries(A.map(e=>[e.id,e])),edges:C,selected:new Set,selectedEdge:null}),Z=e=>({...e,layer:"screen"});function ee(e){const t=Object.values(e.tables);return z("root",{marquee:(n,s)=>({kind:"marquee",rect:n,mods:s}),clear:{kind:"clear"},keys:{Delete:()=>({kind:"delete"}),a:n=>n.ctrl?{kind:"select-all"}:void 0,l:()=>({kind:"arrange"})}},[R("graph",{},[...e.edges.map(n=>L(n.id,{id:n.id,from:n.from,to:n.to,select:s=>({kind:"select-edge",id:s}),states:{sel:e.selectedEdge===n.id,hot:e.selected.has(c(n.from))||e.selected.has(c(n.to))}})),...t.map(n=>F(n.id,{id:n.id,name:n.name,hue:n.hue,pos:n.pos,columns:n.columns,select:(s,r)=>({kind:"select",id:s,mods:r}),drag:(s,r)=>({kind:"drag",id:s,delta:r}),connect:(s,r)=>({kind:"connect",a:s,b:r}),states:{sel:e.selected.has(n.id)}}))]),Z(T("hud",{},[D("panel",{pad:12,gap:8},[g("title",{text:"Schema graph",size:15,weight:600,bright:!0}),P("actions",{gap:6},[l("arrange",{label:"Arrange",accent:!0,press:{kind:"arrange"}}),l("delete",{label:"Delete",danger:!0,press:{kind:"delete"}}),l("reset",{label:"Reset",press:{kind:"reset"}}),l("undo",{label:"Undo",press:{kind:"undo"}}),l("redo",{label:"Redo",press:{kind:"redo"}})]),g("sel",{text:`${e.selected.size} selected · ${e.edges.length} foreign keys · ${t.length} tables`,dim:!0,size:12}),g("hint",{text:"drag tables · marquee on empty · Alt-drag pans · wheel zooms · port → port adds a key · click a wire + Delete · l arranges",dim:!0,size:11})]),j("dock",{corner:"bottom-right"},[_(H("map",{items:t.map(n=>({id:n.id,rect:x(n),hue:n.hue,selected:e.selected.has(n.id)}))}),d(0,0))])]))])}const ne=document.getElementById("c");O(ne,I({init:v,update:Q,view:ee}));N([{name:"main.ts",code:K},{name:"parts.ts",code:G},{name:"marquee.ts (shared)",code:U},{name:"graph-layout.ts (shared)",code:Y},{name:"minimap.ts (shared)",code:X},{name:"sample-schema.ts (shared)",code:J}]);
