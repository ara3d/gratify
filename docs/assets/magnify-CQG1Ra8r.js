...
`,r=60,G=42,o=115,U=2.5,X=o*.16,z=e=>e*e*(3-2*e),k=M("magnify-stage",{size:()=>i(0,0),render(e,t){const n=e.rect,l=e.time??0,g=o+20,d=o+20,S=i(n.center.x+(n.w/2-g)*Math.sin(l*.7),n.center.y+(n.h/2-d)*Math.sin(l*1.06+1.3)),a=e.pointer?i(E(e.pointer.x,n.x,n.right),E(e.pointer.y,n.y,n.bottom)):S,u=Math.ceil(n.w/r),p=Math.ceil(n.h/r),N=n.x+(n.w-(u-1)*r)/2,x=n.y+(n.h-(p-1)*r)/2;let w=0;for(let I=0;I<p;I++)for(let y=0;y<u;y++){w++;const s=i(N+y*r,x+I*r),h=R(s,a);let b=s,f=1;if(h<o){const _=h/o;if(f=1+(U-1)*(1-z(_)),h>.001){const v=i((s.x-a.x)/h,(s.y-a.y)/h),A=(f-1)*X;b=i(s.x+v.x*A,s.y+v.y*A)}}Y(t,b,G*f,w,f)}t.dot(a,o,m(c(255,255,255),.05)),t.ring(a,o,m(c(255,255,255),.85),3),t.ring(a,o-4,m(c(120,190,255),.5),2),t.dot(i(a.x-o*.35,a.y-o*.4),6,m(c(255,255,255),.7))}});function Y(e,t,n,l,g){const d=n/2,S=D(t.x-d,t.y-d,n,n),a=l*33%360,u=.42+.12*(g-1),p=P(a,.55,u);e.box(S,Math.max(3,n*.18),p,m(c(255,255,255),.15*g),1),e.label(String(l),t,c(255,255,255),{size:Math.max(9,n*.32),weight:600})}const F=document.getElementById("c");L(F,{init:{},update:(e,t)=>e,view:()=>k("root",{}),ambient:()=>!0});T([{name:"main.ts",code:C}]);
